<? $temp = null;?>
<form action="<?= site_url();?>/admin/Content/editContentAJAX/<?= $post->content_id;?>" method="POST"
	autocomplete="off" id="content_form">
	<fieldset>
		<label><?=ucwords($lang['edit_contents']);?></label>
		<fieldset>
			<section>
				...
				<div>
					<input type="text" class="text" name="content_title" id="content_title"
						...
						value="<?= $post->content_title;?>" /> <input type="hidden" name="content_author"
			...
			<section>
				...
				<div>
					<textarea class="html" id="content_content" name="content" rows="10" cols="55"
						placeholder="<?= ucwords($lang['content']);?> " required value=""><?= $post->content;?></textarea>
				</div>
			</section>
		</fieldset>
		<fieldset>
			...
			<section>
				...
				<div id="categories">
						...
						value="<?= $cat->idCategories;?>" checked="checked"> <span><?= $cat->Name;?></span>
						<? $temp[] .= $cat->idCategories;?>
						<? endif;?>
						<? endforeach;?>
					</div>
				...
				<div id="add_category_" style="display: none;">
					<input type="text" id="new_category" value=""
						placeholder="<?= ucwords($lang['new_cat']);?>" disabled="true" /> <a href="#"
						id="save_cat"><?= ucwords($lang['save_a_cat']);?></a><a href="#"
						id="save_cat_cancel"><?= ucwords($lang['cancel']);?></a>
					<div id="categories_add">
						<? foreach(Functions::get_all_cats($temp) as $cat) :?>
						<input type="checkbox" class="checkbox" id="<?= $cat->Slug;?>" name="cats[]"
							value="<?= $cat->idCategories;?>"> <span><?= $cat->Name;?></span>
						<? endforeach;?>
					</div>
				</div>
			</section>
		...
		<fieldset>
			<section>
				<div>
					<input type="hidden" name="content_status" id="content_status" value="<?= $post->content_status;?>" />
					<?=ucwords($lang['content_status']);?>: <span id="content_status_"><?= ucwords($lang[Functions::content_status($post->content_status)]);?></span>
				...
				<div>
					<input type="hidden" name="content_type" id="content_type" value="<?= $post->content_type;?>" />
					<?=ucwords($lang['content_type']);?>: <span id="content_type_"><?= ucwords($lang[Functions::content_type($post->content_type)]);?></span>
				...
				<div>
					<input type="hidden" name="content_visibility" id="content_visibility" value="<?= $post->content_visibility;?>" />
				<?=ucwords($lang['visibility']);?>: <span id="content_visibility_"><?= ucwords($lang[Functions::content_visibility($post->content_visibility)]);?></span>
...
$(document).ready(function() {
	...
	$('#add_category').click(function(e) {
		...
		$('#categories_add input[type="checkbox"]').click(function(e) {
			if ($(this).is(':checked')) {
				$('#categories').append('<input type="checkbox" class="checkbox" id="' + $(this).attr('id') + '" name="cats[]" value="' + $(this).val() +'" checked="checked"> <span>' + $(this).next('span').html() + '</span>');
...
function showRequest(formData, jqForm, options) { 
    ...
    if (!$('#content_title').val() || !$('#content_content').val()) {
        alert('FALSE NOT FILLED');//TODO: message
        return false;
    }
